...
const isOverLap = (zoneA: string, zoneB: string) => {
  ...
  const rightPointInLeft =
    isNumberInRange(zoneALeft, zoneARight, zoneBLeft) || isNumberInRange(zoneALeft, zoneARight, zoneBRight);

  const leftPointInRight =
    isNumberInRange(zoneBLeft, zoneBRight, zoneALeft) || isNumberInRange(zoneBLeft, zoneBRight, zoneARight);

  return rightPointInLeft || leftPointInRight;
};

const run = () => {
  let containedTotal = 0,
    overLapTotal = 0;
